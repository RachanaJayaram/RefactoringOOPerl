...
#define SvEVALED(sv) \
    (SvTYPE(sv) >= SVt_PVNV && ((XPVIV *)SvANY(sv))->xiv_u.xivu_eval_seen)
 ...
 */
bool Perl_feature_is_enabled(pTHX_ const char *const name, STRLEN namelen)
{
...
void Perl_lex_start(pTHX_ SV *line, PerlIO *rsfp, U32 flags)
{
    ...
    *parser->lex_casestack = '\0';
    ...
    if (line)
    {
        ...
        if (SvUTF8(line) && UNLIKELY(!is_utf8_string_loc((U8 *)s,
                                                         SvCUR(line),
                                                         &first_bad_char_loc)))
        {
    ...
    STATIC_ASSERT_STMT(FITS_IN_8_BITS(LEX_IGNORE_UTF8_HINTS | LEX_EVALBYTES | LEX_DONT_CLOSE_RSFP));
    parser->lex_flags = (U8)(flags & (LEX_IGNORE_UTF8_HINTS | LEX_EVALBYTES | LEX_DONT_CLOSE_RSFP));
...
void Perl_parser_free(pTHX_ const yy_parser *parser)
{
    ...
    else if (parser->rsfp && (!parser->old_parser || (parser->old_parser && parser->rsfp != parser->old_parser->rsfp)))
...
void Perl_parser_free_nexttoke_ops(pTHX_ yy_parser *parser, OPSLAB *slab)
{
    I32 nexttoke = parser->nexttoke;
    PERL_ARGS_ASSERT_PARSER_FREE_NEXTTOKE_OPS;
    while (nexttoke--)
    {
        if (S_is_opval_token(parser->nexttype[nexttoke] & 0xffff) && parser->nextval[nexttoke].opval && parser->nextval[nexttoke].opval->op_slabbed && OpSLAB(parser->nextval[nexttoke].opval) == slab)
        {
...
Perl_lex_grow_linestr(pTHX_ STRLEN len)
{
    ...
    current = (!PL_parser->lex_shared->ls_linestr || linestr == PL_parser->lex_shared->ls_linestr);
    ...
    re_eval_start_pos = (current && PL_parser->lex_shared->re_eval_start) ? PL_parser->lex_shared->re_eval_start - buf : 0;
...
void Perl_lex_stuff_pvn(pTHX_ const char *pv, STRLEN len, U32 flags)
{
    dVAR;
    char *bufptr;
...
bool Perl_lex_next_chunk(pTHX_ U32 flags)
{
    ...
    linestr = PL_parser->linestr;
    buf = SvPVX(linestr);
    if (!(flags & LEX_KEEP_PREVIOUS) && PL_parser->bufptr == PL_parser->bufend)
    {
    ...
    if (got_some_for_debugger && PERLDB_LINE_OR_SAVESRC && PL_curstash != PL_debstash)
    {
...
bool Perl_validate_proto(pTHX_ SV *name, SV *proto, bool warn, bool curstash)
{
    STRLEN len, origlen;
    char *p;
        ...
        if (!isSPACE(*p))
        {
            ...
            else
            {
                if (*p == '[')
                    ...
                    in_brackets = FALSE;
                else if ((*p == '@' || *p == '%') && !after_slash && !in_brackets)
                {
                    must_be_last = TRUE;
                    greedy_proto = *p;
                }
...
S_incline(pTHX_ const char *s, const char *end)
{
    ...
    COPLINE_INC_WITH_HERELINES;
    if (!PL_rsfp && !PL_parser->filtered && PL_lex_state == LEX_NORMAL && s + 1 == PL_bufend && *s == ';')
    {
...
S_skipspace_flags(pTHX_ char *s, U32 flags)
{
    ...
    else
    {
        ...
        lex_read_space(flags | LEX_KEEP_PREVIOUS |
                       (PL_lex_inwhat || PL_lex_state == LEX_FORMLINE ? LEX_NO_NEXT_CHUNK : 0));
        s = PL_bufptr;
...
S_postderef(pTHX_ int const funny, char const next)
{
    ...
    {
        if ('@' == funny && PL_lex_state == LEX_INTERPNORMAL && !PL_lex_brackets)
            PL_lex_dojoin = 2;
        PL_expect = XOPERATOR;
        PL_bufptr++;
    }
...
S_newSV_maybe_utf8(pTHX_ const char *const start, STRLEN len)
{
    SV *const sv = newSVpvn_utf8(start, len,
                                 !IN_BYTES && UTF && is_utf8_non_invariant_string((const U8 *)start, len));
    return sv;
}
...
S_force_word(pTHX_ char *start, int token, int check_keyword, int allow_pack)
{
    ...
    s = start;
    if (isIDFIRST_lazy_if_safe(s, PL_bufend, UTF) || (allow_pack && *s == ':' && s[1] == ':'))
    {
...
S_force_ident(pTHX_ const char *s, int kind)
{
    ...
    if (s[0])
    {
        ...
        OP *const o = newSVOP(OP_CONST, 0, newSVpvn_flags(s, len, UTF ? SVf_UTF8 : 0));
	       ...
	       GSAR 96-10-12 */
            gv_fetchpvn_flags(s, len,
                              (PL_in_eval ? GV_ADDMULTI
                                          : GV_ADD) |
                                  (UTF ? SVf_UTF8 : 0),
                              kind == '$' ? SVt_PV : kind == '@' ? SVt_PVAV : kind == '%' ? SVt_PVHV : SVt_PVGV);
        }
    }
}
...
S_force_strict_version(pTHX_ char *s)
{
    ...
    else if ((*s != ';' && *s != '{' && *s != '}') && (s = skipspace(s), (*s != ';' && *s != '{' && *s != '}')))
    {
        PL_bufptr = s;
        if (errstr)
    ...
    S_sublex_done(pTHX)
{
    ...
    if (PL_lex_casemods)
    { /* oops, we've got some unbalanced parens */
    ...
    if (PL_lex_repl)
    {
        ...
        *PL_lex_casestack = '\0';
        ...
        {
            CopLINE(PL_curcop) +=
                ((XPVNV *)SvANY(PL_linestr))->xnv_u.xnv_lines + PL_parser->herelines;
            PL_parser->herelines = 0;
        }
        return '/';
    }
...
S_get_and_check_backslash_N_name(pTHX_ const char *s, const char *const e)
{
    ...
    if (cvp && (cv = *cvp) && SvROK(cv) && (rv = SvRV(cv), SvTYPE(rv) == SVt_PVCV) && ((stash = CvSTASH(rv)) != NULL))
    {
...
S_scan_const(pTHX_ char *start)
{
    ...
    while (s < send || dorange /* Handle tr/// range at right edge of input */
    )
    {
        ...
        if (PL_lex_inwhat == OP_TRANS)
        {
            ...
            if (!dorange)
            {
                ...
                { /* Is a '-' in the context where it means a range */
                    if (didrange)
                    { /* Something like y/A-C-Z// */
            ...
            else
            {
                ...
                convert_unicode =
                    ...
                    || (!non_portable_endpoint && ((isLOWER_A(range_min) && isLOWER_A(range_max)) || (isUPPER_A(range_min) && isUPPER_A(range_max))));
                if (convert_unicode)
                {
                ...
                if (range_min > range_max)
                {
                    ...
                    else if (convert_unicode)
                    {
                        ...
                        Perl_croak(aTHX_
                                   "Invalid range \"\\N{U+%04" UVXf "}-\\N{U+%04" UVXf "}\" in transliteration operator",
                                   range_min, range_max);
                    }
#endif
                    else
                ...
                d = offset_to_min + SvGROW(sv, SvCUR(sv) + (send - s) + grow + 1 /* Trailing NUL */);
        ...
        else if (*s == '(' && PL_lex_inpat && s[1] == '?' && !in_charclass)
        {
            ...
            else if (!PL_lex_casemods && (s[2] == '{' /* This should match regcomp.c */
                                          || (s[2] == '?' && s[3] == '{')))
            {
                break;
            }
        ...
        else if (*s == '#' && PL_lex_inpat && !in_charclass && ((PMOP *)PL_lex_inpat)->op_pmflags & RXf_PMf_EXTENDED)
        {
            while (s < send && *s != '\n')
                *d++ = *s++;
        }
	     ...
	     * is an octal; and \19 is \1 followed by '9' */
            if (PL_lex_inwhat == OP_SUBST && !PL_lex_inpat && isDIGIT(*s) && *s != '0' && !isDIGIT(s[1]))
            {
	     ...
	     * quantifier */
            else if (PL_lex_inpat && (*s != 'N' || s[1] != '{' || regcurly(s + 1)))
            {
                *d++ = '\\';
                goto default_action;
            }
            ...
            case '7':
            {
                ...
                s += len;
                if (len < 3 && s < send && isDIGIT(*s) && ckWARN(WARN_MISC))
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        * platforms, but be safe.  See Note on sizing above. */
                        const STRLEN needed = d - SvPVX(sv) + UVCHR_SKIP(uv) + (send - s) + 1;
		 ...
		 * succeed if we are being called on a pattern, as we already
                ...
                {           /* \N{U+...} */
                    ...
                    { /* Not a pattern: convert the hex to string */
                        I32 flags = PERL_SCAN_ALLOW_UNDERSCORES | PERL_SCAN_SILENT_ILLDIGIT | PERL_SCAN_DISALLOW_PREFIX;
                        STRLEN len = e - s;
                        ...
                        /* For non-tr///, if the destination is not in utf8,
                          ...
                          * points */
                        if (!has_utf8 && (uv > 0xFF || PL_lex_inwhat != OP_TRANS))
                        {
                            /* See Note on sizing above.  */
                    ...
                    if ((res = get_and_check_backslash_N_name(s, e)))
                {
                    ...
                    if (PL_lex_inpat)
                    {
                        ...
                        if (!len)
                        { /* The name resolved to an empty string */
                            Copy("\\N{}", d, 4, char);
                            d += 4;
                        }
                        else
			    ...
			    * returned by charnames */
                            ...
                            if (!SvUTF8(res))
                            {
                                ...
                                const STRLEN initial_len = sizeof(initial_text) - 1;
                                d = off + SvGROW(sv, off + 3 * len
                            ...
                            else
                            {
                                ...
                                d = off + SvGROW(sv, off + output_length + (STRLEN)(send - e) + 2); /* '}' + NUL */
                                /* And output it */
                    ...
                    { /* Here, not in a pattern.  Convert the name to a
			    * string. */
                        ...
                        else if (len > (STRLEN)(e - s + 4))
                        { /* I _guess_ 4 is \N{} --jhi */
    ...
    default_action:
        ...
        { /* UTF8ness matters and doesn't match, need to convert */
            STRLEN len = 1;
    ...
    *d = '\0';
    ...
    if (s > start)
    {
        ...
        if ((PL_hints & (PL_lex_inpat ? HINT_NEW_RE : HINT_NEW_STRING)) && !PL_parser->lex_re_reparsing)
        {
...
S_intuit_more(pTHX_ char *s, char *e)
{
    ...
    if (*s == '-' && s[1] == '>' && (s[2] == '[' || s[2] == '{'))
        return TRUE;
    if (*s == '-' && s[1] == '>' && FEATURE_POSTDEREF_QQ_IS_ENABLED && ((s[2] == '$' && (s[3] == '*' || (s[3] == '#' && s[4] == '*'))) || (s[2] == '@' && strchr("*[{", s[3]))))
        return TRUE;
    if (*s != '{' && *s != '[')
        return FALSE;
    ...
    else
    {
        ...
        for (; s < send; s++)
        {
            ...
            case '-':
                ...
                break;
            default:
                if (!isWORDCHAR(last_un_char) && !(last_un_char == '$' || last_un_char == '@' || last_un_char == '&') && isALPHA(*s) && s[1] && isALPHA(s[1]))
                {
                    char *d = s;
                    while (isALPHA(*s))
                        s++;
...
S_intuit_method(pTHX_ char *start, SV *ioname, CV *cv)
{
    ...
    if (*start == '$')
    {
        ...
        if (cv || PL_last_lop_op == OP_PRINT || PL_last_lop_op == OP_SAY || isUPPER(*PL_tokenbuf))
            return 0;
        /* this could be $# */
        if (isSPACE(*s))
            s = skipspace(s);
    ...
    if (!keyword(tmpbuf, len, 0))
    {
        ...
        indirgv = gv_fetchpvn_flags(tmpbuf, len,
                                    ...
                                    SVt_PVCV);
        if (indirgv && SvTYPE(indirgv) != SVt_NULL && (!isGV(indirgv) || GvCVu(indirgv)))
            return 0;
...
SV *Perl_filter_add(pTHX_ filter_t funcp, SV *datasv)
{
    ...
    if (
        !PL_parser->filtered && PL_parser->lex_flags & LEX_EVALBYTES && PL_bufptr < PL_bufend)
    {
...
I32 Perl_filter_read(pTHX_ int idx, SV *buf_sv, int maxlen)
{
    ...
    if (!PL_parser || !PL_rsfp_filters)
        return -1;
    if (idx > AvFILLp(PL_rsfp_filters))
    { /* Any more filters?	*/
    ...
    if (SvTYPE(datasv) != SVt_PVIO)
    {
        if (correct_length)
        {
            ...
            if (!remainder)
                return 0; /* eof */
            if (correct_length > remainder)
                correct_length = remainder;
...
S_find_in_my_stash(pTHX_ const char *pkgname, STRLEN len)
{
    ...
    if (len > 2 && (pkgname[len - 2] == ':' && pkgname[len - 1] == ':') && (gv = gv_fetchpvn_flags(pkgname, len, (UTF ? SVf_UTF8 : 0), SVt_PVHV)))
    {
        return GvHV(gv); /* Foo:: */
    }
...
{
    return (len == 1 && strchr("msyq", p[0])) || (len == 2 && ((p[0] == 't' && p[1] == 'r') || (p[0] == 'q' && strchr("qwxr", p[1]))));
}
...
S_check_scalar_slice(pTHX_ char *s)
{
    ...
    while (SPACE_OR_TAB(*s))
        s++;
    if (*s == 'q' && s[1] == 'w' && !isWORDCHAR_lazy_if_safe(s + 2, PL_bufend, UTF))
    {
        return;
    }
    while (isWORDCHAR_lazy_if_safe(s, PL_bufend, UTF) || (*s && strchr(" \t$#+-'\"", *s)))
    {
        s += UTF ? UTF8SKIP(s) : 1;
    }
    if (*s == '}' || *s == ']')
    ...
    Perl_yylex(pTHX)
{
    ...
    */
    case LEX_INTERPCASEMOD:
        ...
        if (PL_bufptr == PL_bufend || PL_bufptr[1] == 'E')
        {
            ...
            if (PL_lex_casemods)
            {
                ...
                if (PL_bufptr != PL_bufend && (oldmod == 'L' || oldmod == 'U' || oldmod == 'Q' || oldmod == 'F'))
                {
                    PL_bufptr += 2;
                    PL_lex_state = LEX_INTERPCONCAT;
                }
        ...
        else
        {
            ...
            {
                I32 tmp;
                if (memBEGINs(s, (STRLEN)(PL_bufend - s), "L\\u") || memBEGINs(s, (STRLEN)(PL_bufend - s), "U\\l"))
                {
                    tmp = *s, *s = s[2], s[2] = (char)tmp; /* misordered... */
                }
                if ((*s == 'L' || *s == 'U' || *s == 'F') && (strpbrk(PL_lex_casestack, "LUF")))
                {
                    PL_lex_casestack[--PL_lex_casemods] = '\0';
                    PL_lex_allbrackets--;
                    return REPORT(')');
    ...
    case LEX_INTERPSTART:
        ...
        PL_lex_dojoin = (*PL_bufptr == '@' && (!PL_lex_inpat || PL_lex_casemods));
        PL_lex_state = LEX_INTERPNORMAL;
        if (PL_lex_dojoin)
        {
            NEXTVAL_NEXTTOKE.ival = 0;
    ...
    case LEX_INTERPEND:
        ...
        if (PL_lex_inwhat == OP_SUBST && PL_linestr == PL_lex_repl && SvEVALED(PL_lex_repl))
        {
            if (PL_bufptr != PL_bufend)
	   ...
	   check re_eval_str as well. */
        if (PL_parser->lex_shared->re_eval_start || PL_parser->lex_shared->re_eval_str)
        {
            SV *sv;
            if (*PL_bufptr != ')')
...
retry:
    ...
    default:
        ...
        }
        {
            ...
            if (UTF)
            {
                ...
                c = sv_uni_display(dsv, newSVpvn_flags(s, skiplen > stravail ? stravail : skiplen, SVs_TEMP | SVf_UTF8),
    ...
    case 0:
        if ((!PL_rsfp || PL_lex_inwhat) && (!PL_parser->filtered || s + 1 < PL_bufend))
        {
            PL_last_uni = 0;
            PL_last_lop = 0;
            if (PL_lex_brackets && PL_lex_brackstack[PL_lex_brackets - 1] != XFAKEEOF)
            {
                yyerror((const char *)(PL_lex_formbrack
		 ...
		 * otherwise a compile-time require of perl5db.pl.  */
            ...
            if (PL_minus_n || PL_minus_p)
            {
                ...
                if (PL_minus_a)
                {
                    if (PL_minus_F)
                    {
                        if ((*PL_splitstr == '/' || *PL_splitstr == '\'' || *PL_splitstr == '"') && strchr(PL_splitstr + 1, *PL_splitstr))
                        {
	     ...
	     * check if it in fact is. */
            if (bof && PL_rsfp && (*s == 0 || *(U8 *)s == BOM_UTF8_FIRST_BYTE || *(U8 *)s >= 0xFE || s[1] == 0))
            {
            ...
            {
                /* Incest with pod. */
                if (memBEGINPs(s, (STRLEN)(PL_bufend - s), "=cut") && !isALPHA(s[4]))
                {
                    SvPVCLEAR(PL_linestr);
                    PL_oldoldbufptr = PL_oldbufptr = s = PL_linestart = SvPVX(PL_linestr);
                    PL_bufend = SvPVX(PL_linestr) + SvCUR(PL_linestr);
		 ...
		 */
		     ...
		     * line contains "Perl" rather than "perl" */
                    if (!d)
                    {
                        ...
                        {
                            if (isALPHA_FOLD_EQ(*d, 'p') && !ibcmp(d, "perl", 4))
                            {
                                break;
                            }
                        }
...
#endif /* ALTERNATE_SHEBANG */
                if (!d && *s == '#' && ipathend > ipath && !PL_minus_c && !instr(s, "indir") && instr(PL_origargv[0], "perl"))
                {
                    dVAR;
                    char **newargv;
                ...
                if (d)
                {
                    ...
                    if (*d++ == '-')
                    {
                        ...
                        do
                        {
                            ...
                            {
                                const char *d2 = d1 + 1;
                                if (parse_unicode_opts((const char **)&d2) != PL_unicode)
                                    baduni = TRUE;
                            }
                        ...
                        if ((PERLDB_LINE_OR_SAVESRC && !oldpdb) || ((PL_minus_n || PL_minus_p) && !(oldn || oldp)))
    ...
    case '#':
    case '\n':
        if (PL_lex_state != LEX_NORMAL || (PL_in_eval && !PL_rsfp && !PL_parser->filtered))
        {
            const bool in_comment = *s == '#';
            if (*s == '#' && s == PL_linestart && PL_in_eval && !PL_rsfp && !PL_parser->filtered)
            {
            ...
            if (d < PL_bufend)
                d++;
            s = d;
            if (in_comment && d == PL_bufend && PL_lex_state == LEX_INTERPNORMAL && PL_lex_inwhat == OP_SUBST && PL_lex_repl == PL_linestr && SvEVALED(PL_lex_repl) && d[-1] == '}')
                s--;
            else
                incline(s, PL_bufend);
            if (PL_lex_formbrack && PL_lex_brackets <= PL_lex_formbrack)
    ...
    case '-':
        ...
        }
        {
            ...
            else if (*s == '>')
            {
                s++;
                s = skipspace(s);
                if (((*s == '$' || *s == '&') && s[1] == '*') || (*s == '$' && s[1] == '#' && s[2] == '*') || ((*s == '@' || *s == '%') && strchr("*[{", s[1])) || (*s == '*' && (s[1] == '*' || s[1] == '{')))
                {
    ...
    case '%':
    {
        if (PL_expect == XOPERATOR)
        {
            if (s[1] == '=' && !PL_lex_allbrackets && PL_lex_fakeeof >= LEX_FAKEEOF_ASSIGN)
            {
                TOKEN(0);
            }
            ++s;
        ...
        s++;
        {
             ...
             */
		       ...
		       of applying only built-in attributes.) */
                ...
                {
                    if (*s != ';' && *s != '}' && !(PL_expect == XOPERATOR ? (*s == '=' || *s == ')') : (*s == '{' || *s == '(')))
                    {
                        const char q = ((*s == '\'') ? '"' : '\'');
    ...
    leftbracket:
		 ...
		 */
            ...
            {
                if (++t < PL_bufend && (!isWORDCHAR(*t) || ((*t == 'q' || *t == 'x') && ++t < PL_bufend && !isWORDCHAR(*t))))
                {
                    ...
                    /* skip plain q word */
                    while (t < PL_bufend && isWORDCHAR_lazy_if_safe(t, PL_bufend, UTF))
                    {
                        t += UTF ? UTF8SKIP(t) : 1;
                    }
            }
            ...
            if (t < PL_bufend && ((*t == ',' && (*s == 'q' || !isLOWER(*s))) || (*t == '=' && t[1] == '>')))
                OPERATOR(HASHBRACK);
            if (PL_expect == XREF)
            {
            block_expectation:
    ...
    rightbracket:
        ...
        if (PL_lex_state == LEX_INTERPNORMAL)
        {
            if (PL_lex_brackets == 0)
            {
                ...
                if (PL_lex_inwhat == OP_SUBST && PL_lex_repl == PL_linestr && SvEVALED(PL_lex_repl))
                    PL_lex_state = LEX_INTERPEND;
                else if (*s == '-' && s[1] == '>')
                    PL_lex_state = LEX_INTERPENDMAYBE;
                else if (*s != '[' && *s != '{')
        ...
        if (formbrack)
            LEAVE_with_name("lex_format");
        if (formbrack == 2)
        { /* means . where arguments were expected */
            force_next(';');
            TOKEN(FORMRBRACK);
        }
        TOKEN(';');
    case '&':
        ...
        if (PL_expect == XOPERATOR)
        {
            if (PL_bufptr == PL_linestart && ckWARN(WARN_SEMICOLON) && isIDFIRST_lazy_if_safe(s, PL_bufend, UTF))
            {
                CopLINE_dec(PL_curcop);
    ...
    case '=':
        s++;
        {
            ...
            {
                if ((PL_in_eval && !PL_rsfp && !PL_parser->filtered) || PL_lex_state != LEX_NORMAL)
                {
                    d = PL_bufend;
                    while (s < d)
                    {
    ...
    case '!':
		 ...
		 * warn on m:!=~\s+([/?]|[msy]\W|tr\W): */
                ...
                if (*s == '~' && ckWARN(WARN_SYNTAX))
                {
                    ...
                    if (*t == '/' || *t == '?' || ((*t == 'm' || *t == 's' || *t == 'y') && !isWORDCHAR(t[1])) || (*t == 't' && t[1] == 'r' && !isWORDCHAR(t[2])))
                        Perl_warner(aTHX_ packWARN(WARN_SYNTAX),
                                    "!=~ should be !~");
                }
                if (!PL_lex_allbrackets && PL_lex_fakeeof >= LEX_FAKEEOF_COMPARE)
    ...
    case '$':
        ...
        if (s[1] == '#' && (isIDFIRST_lazy_if_safe(s + 2, PL_bufend, UTF) || strchr("{$:+-@", s[2])))
        {
            PL_tokenbuf[0] = '@';
        ...
        d = s;
        {
            ...
            if ((PL_expect != XREF || PL_oldoldbufptr == PL_last_lop) && intuit_more(s, PL_bufend))
            {
                if (*s == '[')
                {
                    ...
                    if (ckWARN(WARN_SYNTAX))
                    {
                        ...
                        while (isSPACE(*t) || isWORDCHAR_lazy_if_safe(t, PL_bufend, UTF) || *t == '$')
                        {
                            t += UTF ? UTF8SKIP(t) : 1;
                        }
                        if (*t++ == ',')
                ...
                else if (*s == '{')
                {
                    char *t;
                    PL_tokenbuf[0] = '%';
                    if (strEQ(PL_tokenbuf + 1, "SIG") && ckWARN(WARN_SYNTAX) && (t = (char *)memchr(s, '}', PL_bufend - s)) && (t = (char *)memchr(t, '=', PL_bufend - t)))
                    {
                        char tmpbuf[sizeof PL_tokenbuf];
                        do
                        {
                        ...
                        if (isIDFIRST_lazy_if_safe(t, PL_bufend, UTF))
                        {
                            ...
                            while (isSPACE(*t))
                                t++;
                            if (*t == ';' && get_cvn_flags(tmpbuf, len, UTF ? SVf_UTF8 : 0))
                            {
                                Perl_warner(aTHX_ packWARN(WARN_SYNTAX),
            ...
            if (PL_lex_state == LEX_NORMAL && isSPACE((char)tmp))
            {
                ...
                else if (strchr("&*<%", *s) && isIDFIRST_lazy_if_safe(s + 1, PL_bufend, UTF))
                {
    ...
    case '/': /* may be division, defined-or, or pattern */
        ...
        else
        {
            ...
            if (PL_oldoldbufptr == PL_last_uni && (*PL_last_uni != 's' || s - PL_last_uni < 5 || memNE(PL_last_uni, "study", 5) || isWORDCHAR_lazy_if_safe(PL_last_uni + 5, PL_bufend, UTF)))
                check_uni();
    ...
    case '?': /* conditional */
        s++;
        if (!PL_lex_allbrackets && PL_lex_fakeeof >= LEX_FAKEEOF_IFELSE)
        {
            s--;
            TOKEN(0);
        }
    ...
    case '.':
        ...
        if (PL_expect == XOPERATOR || !isDIGIT(s[1]))
        {
            ...
            if (*s == tmp)
            {
                if (!PL_lex_allbrackets && PL_lex_fakeeof >= LEX_FAKEEOF_RANGE)
                {
                    s--;
                    TOKEN(0);
                }
    ...
    case 'v':
        if (isDIGIT(s[1]) && PL_expect != XOPERATOR)
        {
            ...
            else if ((*start == ':' && start[1] == ':') || (PL_expect == XSTATE && *start == ':'))
                goto keylookup;
            ...
            if (!isALPHA(*start) && (PL_expect == XTERM || PL_expect == XREF || PL_expect == XSTATE || PL_expect == XTERMORDORDOR))
            {
    ...
    keylookup:
    {
        ...
        /* Is this a label? */
        if (!anydelim && PL_expect == XSTATE && d < PL_bufend && *d == ':' && *(d + 1) != ':')
        {
            s = d + 1;
        ...
        {                   /* second-class keyword? */
            ...
            if (PL_expect != XOPERATOR && (*s != ':' || s[1] != ':'))
            {
                ...
                if (!ogv && (gvp = (GV **)hv_fetch(PL_globalstash, PL_tokenbuf, len, FALSE)) && (gv = *gvp) && (isGV_with_GP(gv) ? GvCVu(gv) && GvIMPORTED_CV(gv) : SvPCS_IMPORTED(gv) && (gv_init(gv, PL_globalstash, PL_tokenbuf, len, 0), 1)))
                {
                    ogv = gv;
                }
            }
            ...
            else if (gv && !gvp && -tmp == KEY_lock /* XXX generalizable kludge */
                     && GvCVu(gv))
            {
        ...
        if (tmp && tmp != KEY___DATA__ && tmp != KEY___END__ && (!anydelim || *s != '#'))
        {
    ...
    reserved_word:
        ...
        just_a_word:
        {
		   ...
		   (and a package name). */
            ...
            {
                if (ckWARN(WARN_BAREWORD) && !gv_fetchpvn_flags(PL_tokenbuf, len, UTF ? SVf_UTF8 : 0, SVt_PVHV))
		   ...
		   called.  intuit_method returns 0 or > 255.  */
            ...
            if (PL_oldoldbufptr && PL_oldoldbufptr < PL_bufptr && (PL_oldoldbufptr == PL_last_lop || PL_oldoldbufptr == PL_last_uni) && /* NO SKIPSPACE BEFORE HERE! */
                (PL_expect == XREF || ((PL_opargs[PL_last_lop_op] >> OASHIFT) & 7) == OA_FILEREF))
            {
                ...
                if ((isIDFIRST_lazy_if_safe(s, PL_bufend, UTF) || *s == '$') && (tmp = intuit_method(s, lex ? NULL : sv, cv)))
                {
                ...
                if (
                    (!immediate_paren && (PL_last_lop_op == OP_SORT || (!cv && (PL_last_lop_op != OP_MAPSTART && PL_last_lop_op != OP_GREPSTART)))) || (PL_tokenbuf[0] == '_' && PL_tokenbuf[1] == '\0' && ((PL_opargs[PL_last_lop_op] & OA_CLASS_MASK) == OA_FILESTATOP)))
                {
            ...
            if (tmp == 1 && !orig_keyword && (isIDFIRST_lazy_if_safe(s, PL_bufend, UTF) || *s == '$') && (tmp = intuit_method(s, lex ? NULL : sv, cv)))
            {
            method:
                if (lex && !off)
                {
                ...
                op_free(rv2cv_op);
                if (tmp == METHOD && !PL_lex_allbrackets && PL_lex_fakeeof > LEX_FAKEEOF_LOWLOGIC)
                {
                    PL_lex_fakeeof = LEX_FAKEEOF_LOWLOGIC;
                }
                return REPORT(tmp);
		     ...
		     * or exec, which can't be a bareword */
                if ((PL_last_lop_op == OP_PRINT || PL_last_lop_op == OP_PRTF || PL_last_lop_op == OP_SAY || PL_last_lop_op == OP_SYSTEM || PL_last_lop_op == OP_EXEC) && (PL_hints & HINT_STRICT_SUBS))
                    pl_yylval.opval->op_private |= OPpCONST_STRICT;
                if (lastchar != '-')
                {
        ...
        safe_bareword:
            if ((lastchar == '*' || lastchar == '%' || lastchar == '&') && saw_infix_sigil)
            {
                Perl_ck_warner_d(aTHX_ packWARN(WARN_AMBIGUOUS),
		 ...
		 * XXX this is a questionable hack at best. */
                if (PL_bufend - PL_bufptr > 2 && PL_bufend[-1] == '\n' && PL_bufend[-2] == '\r')
                {
                    Off_t loc = 0;
        ...
        case_KEY_CORE:
        {
            ...
            s += 2;
            s = scan_word(s, PL_tokenbuf, sizeof PL_tokenbuf, FALSE, &len);
            if ((*s == ':' && s[1] == ':') || (!(tmp = keyword(PL_tokenbuf, len, 1)) && *s == '\''))
            {
                s = d;
                len = olen;
            ...
            if (tmp < 0)
                tmp = -tmp;
            else if (tmp == KEY_require || tmp == KEY_do || tmp == KEY_glob)
            ...
            if (*s != '\'')
            {
                *PL_tokenbuf = '&';
                d = scan_word(s, PL_tokenbuf + 1, sizeof PL_tokenbuf - 1,
                              1, &len);
                if (len && memNEs(PL_tokenbuf + 1, len, "CORE") && !keyword(PL_tokenbuf + 1, len, 0))
                {
        ...
        case KEY_state:
            if (PL_in_my)
            {
                ...
                yyerror(Perl_form(aTHX_
                                  ...
                                  tmp == KEY_my ? "my" : tmp == KEY_state ? "state" : "our",
                                  PL_in_my == KEY_my ? "my" : PL_in_my == KEY_state ? "state" : "our"));
            }
            PL_in_my = (U16)tmp;
            s = skipspace(s);
            if (isIDFIRST_lazy_if_safe(s, PL_bufend, UTF))
        ...
        case KEY_open:
            ...
            if (isIDFIRST_lazy_if_safe(s, PL_bufend, UTF))
            {
                ...
                if (*t && strchr("|&*+-=!?:.", *t) && ckWARN_d(WARN_PRECEDENCE)
                    /* [perl #16184] */
                    && !(t[0] == '=' && t[1] == '>') && !(t[0] == ':' && t[1] == ':') && !keyword(s, d - s, 0))
                {
        ...
        case KEY_require:
            ...
            else if (*s != 'v' || !isDIGIT(s[1]) || (s = force_version(s, TRUE), *s == 'v'))
            {
                *PL_tokenbuf = '\0';
        ...
        really_sub:
        {
            ...
            if (isIDFIRST_lazy_if_safe(s, PL_bufend, UTF) || *s == '\'' || (*s == ':' && s[1] == ':'))
            {
                ...
                *PL_tokenbuf = '&';
                if (memchr(tmpbuf, ':', len) || key != KEY_sub || pad_findmy_pvn(PL_tokenbuf, len + 1, 0) != NOT_IN_PAD)
            ...
            if (key == KEY_format)
            {
                if (format_name)
                {
                    NEXTVAL_NEXTTOKE.opval = newSVOP(OP_CONST, 0, format_name);
                    NEXTVAL_NEXTTOKE.opval->op_private |= OPpCONST_BARE;
                    force_next(BAREWORD);
                }
                PREBLOCK(FORMAT);
    ...
    S_pending_ident(pTHX)
{
    ...
    */
    ...
    /*
       ...
       table.
    */
    if (ckWARN(WARN_AMBIGUOUS) && pit == '@' && PL_lex_state != LEX_NORMAL && !PL_lex_brackets)
    {
    ...
    if (pit != '&')
        gv_fetchpvn_flags(PL_tokenbuf + 1, tokenbuf_len > 0 ? tokenbuf_len - 1 : 0,
                          (PL_in_eval ? GV_ADDMULTI : GV_ADD) | (UTF ? SVf_UTF8 : 0),
                          ((PL_tokenbuf[0] == '$') ? SVt_PV
                                                   : (PL_tokenbuf[0] == '@') ? SVt_PVAV
                                                                             : SVt_PVHV));
    return BAREWORD;
               ...
               SV *sv, SV *pv, const char *type, STRLEN typelen)
{
    ...
    if (!table || !(PL_hints & HINT_LOCALIZE_HH) || !(cvp = hv_fetch(table, key, keylen, FALSE)) || !SvOK(*cvp))
    {
        char *msg;
              ...
              bool is_utf8, bool check_dollar, bool tick_warn)
{
    ...
    while (*s < PL_bufend)
    {
        ...
        else if (allow_package && **s == '\'' && isIDFIRST_lazy_if_safe((*s) + 1, PL_bufend, is_utf8))
        {
            *(*d)++ = ':';
            *(*d)++ = ':';
            (*s)++;
    ...
    if (UNLIKELY(tick_warn && saw_tick && PL_lex_state == LEX_INTERPNORMAL && !PL_lex_brackets && ckWARN(WARN_SYNTAX)))
    {
        char *d;
        char *d2;
...
#define VALID_LEN_ONE_IDENT(s, e, is_utf8)               \
    (isGRAPH_A(*(s)) || ((is_utf8)                       \
                             ...
                             : (isGRAPH_L1(*s) && LIKELY((U8) * (s) != LATIN1_TO_NATIVE(0xAD)))))
...
S_scan_ident(pTHX_ char *s, char *dest, STRLEN destlen, I32 ck_uni)
{
    ...
    *d = '\0';
    ...
    if (*s == '$' && s[1] && (isIDFIRST_lazy_if_safe(s + 1, PL_bufend, is_utf8) || isDIGIT_A((U8)s[1]) || s[1] == '$' || s[1] == '{' || memBEGINs(s + 1, (STRLEN)(PL_bufend - (s + 1)), "::")))
    {
    ...
    if (bracket != -1)
    {
        ...
        if (isIDFIRST_lazy_if_safe(d, e, is_utf8) || (!isPRINT(*d) /* isCNTRL(d), plus all non-ASCII */
                                                      && isWORDCHAR(*s)))
        {
        ...
        if (*s2 == '}')
        {
            ...
            if (PL_lex_state == LEX_NORMAL)
            {
                if (ckWARN(WARN_AMBIGUOUS) && (keyword(dest, d - dest, 0) || get_cvn_flags(dest, d - dest, is_utf8 ? SVf_UTF8 : 0)))
                {
...
S_pmflag(pTHX_ const char *const valid_flags, U32 *pmfl, char **s, char *charset, unsigned int *x_mod_count)
{
    ...
    case KEEPCOPY_PAT_MOD:
        *pmfl |= RXf_PMf_KEEPCOPY;
        break;
    case NONDESTRUCT_PAT_MOD:
        *pmfl |= PMf_NONDESTRUCT;
        break;
    case LOCALE_PAT_MOD:
        if (*charset)
        {
    ...
    case ASCII_RESTRICT_PAT_MOD:
		 ...
		 * if, and accept, a second occurrence (only) */
            if (*charset != 'a' || get_regex_charset(*pmfl) != REGEX_ASCII_RESTRICTED_CHARSET)
            {
                goto multiple_charsets;
            }
    ...
    *d = '\0';
    ...
    if (!infile || PL_lex_inwhat)
    {
	   ...
	   */
        ...
        {
            while (s < bufend - len + 1 && memNE(s, PL_tokenbuf, len))
            {
                if (*s++ == '\n')
                    ++PL_parser->herelines;
            }
	 ...
	 */
	   ...
	   check shared->re_eval_str. */
        ...
        if (cxstack_ix >= 0 && CxTYPE(cx) == CXt_EVAL && CxOLD_OP_TYPE(cx) == OP_ENTEREVAL && cx->blk_eval.cur_text == linestr)
        {
    ...
    streaming:
        ...
        while (1)
        {
            ...
            if (!lex_next_chunk(LEX_NO_TERM) && (!SvCUR(tmpstr) || SvEND(tmpstr)[-1] != '\n'))
            {
	       ...
	       restore PL_linestr. */
            ...
            {
                if (*s == term && PL_bufend - s >= len && memEQ(s, PL_tokenbuf + 1, len))
                {
                    SvREFCNT_dec(PL_linestr);
                    PL_linestr = linestr_save;
                    PL_linestart = SvPVX(linestr_save);
    ...
    if (indented && indent)
    {
        ...
        while (ss < se)
        {
            ...
            else if (se - ss >= indent_len && memEQ(ss, indent, indent_len))
            {
                STRLEN le = 0;
           ...
           char **delimp)
{
    ...
    else
    {
        ...
        if (check_grapheme)
        {
            if (UNLIKELY(UNICODE_IS_SUPER(termcode)) || UNLIKELY(UNICODE_IS_NONCHAR(termcode)))
            {
    ...
    for (;;)
    {
        ...
        if (PL_multi_open == PL_multi_close)
        {
            for (; s < PL_bufend; s++, to++)
            {
                ...
                {
                    if (!keep_bracketed_quoted && (s[1] == term || (re_reparse && s[1] == '\\')))
                        s++;
		   ...
		   have found the terminator */
                ...
                {                     /* First byte of terminator matches */
                    ...
                    if (s + termlen <= PL_bufend && memEQ(s + 1, (char *)termstr + 1, termlen - 1))
                    {
                        if (check_grapheme && UNLIKELY(!_is_grapheme((U8 *)start,
                                                                     (U8 *)s,
                                                                     (U8 *)PL_bufend,
                                                                     termcode)))
                        {
	...
	*/
        else
        {
            ...
            for (; s < PL_bufend; s++, to++)
            {
                ...
                if (*s == '\\' && s + 1 < PL_bufend)
                {
                    if (!keep_bracketed_quoted && (((UV)s[1] == PL_multi_open) || ((UV)s[1] == PL_multi_close)))
                    {
                        s++;
                    }
                    else
    ...
    */
    ...
    if (PL_lex_stuff)
        ...
        PL_lex_stuff = sv;
    if (delimp)
        *delimp = PL_multi_open == PL_multi_close ? s - termlen : s;
    return s;
}
...
#define HEXFP_NV
		    ...
		    */
            ...
            digit:
                ...
                if (!overflowed)
                {
                    ...
                    if ((x >> shift) != u && !(PL_hints & HINT_NEW_BINARY))
                    {
                        overflowed = TRUE;
                        n = (NV)u;
                ...
                if (isDIGIT(*h))
                {
                    ...
                    while (isDIGIT(*h) || *h == '_')
                    {
                        if (isDIGIT(*h))
                        {
                            ...
                            hexfp_exp += *h - '0';
#ifdef NV_MIN_EXP
                            if (negexp && -hexfp_exp < NV_MIN_EXP - 1)
                            {
...
#endif
#ifdef NV_MAX_EXP
                            if (!negexp && hexfp_exp > NV_MAX_EXP - 1)
                            {
                                Perl_ck_warner(aTHX_ packWARN(WARN_OVERFLOW),
    ...
    */
    ...
    decimal:
        ...
        while (isDIGIT(*s) || *s == '_' || UNLIKELY(hexfp && isXDIGIT(*s)))
        {
	...
	*/
	    ...
	    */
            for (; isDIGIT(*s) || *s == '_' || UNLIKELY(hexfp && isXDIGIT(*s));
                 s++)
            {
        ...
        if ((isALPHA_FOLD_EQ(*s, 'e') || UNLIKELY(hexfp && isALPHA_FOLD_EQ(*s, 'p'))) && strchr("+-0123456789_", s[1]))
        {
            int exp_digits = 0;
            const char *save_s = s;
            char *save_d = d;
            ...
            while (isDIGIT(*s) || *s == '_')
            {
                ...
                {
                    if (((lastub && s == lastub + 1) || (!isDIGIT(s[1]) && s[1] != '_')))
                        WARN_ABOUT_UNDERSCORE();
                    lastub = s++;
                }
            }
...
S_scan_formline(pTHX_ char *s)
{
    ...
    while (!needargs)
    {
        ...
        s = (char *)eol;
        if ((PL_rsfp || PL_parser->filtered) && PL_parser->form_lex_state == LEX_NORMAL)
        {
            bool got_some;
            PL_bufptr = PL_bufend;
            COPLINE_INC_WITH_HERELINES;
...
void Perl_init_named_cv(pTHX_ CV *cv, OP *nameop)
{
    ...
    if (nameop->op_type == OP_CONST)
    {
        ...
        if (strEQ(name, "BEGIN") || strEQ(name, "END") || strEQ(name, "INIT") || strEQ(name, "CHECK") || strEQ(name, "UNITCHECK"))
            CvSPECIAL_on(cv);
    }
    else
        /* State subs inside anonymous subs need to be
     clonable themselves. */
        if (CvANON(CvOUTSIDE(cv)) || CvCLONE(CvOUTSIDE(cv)) || !PadnameIsSTATE(PadlistNAMESARRAY(CvPADLIST(CvOUTSIDE(cv)))[nameop->op_targ]))
        CvCLONE_on(cv);
}
...
void Perl_abort_execution(pTHX_ const char *const msg, const char *const name)
{
...
int Perl_yyerror(pTHX_ const char *const s)
{
    PERL_ARGS_ASSERT_YYERROR;
    return yyerror_pvn(s, strlen(s), 0);
}

int Perl_yyerror_pv(pTHX_ const char *const s, U32 flags)
{
    PERL_ARGS_ASSERT_YYERROR_PV;
    return yyerror_pvn(s, strlen(s), flags);
}

int Perl_yyerror_pvn(pTHX_ const char *const s, STRLEN len, U32 flags)
{
    ...
    if (s != NULL)
    {
        ...
        else if (PL_oldoldbufptr && PL_bufptr > PL_oldoldbufptr && PL_bufptr - PL_oldoldbufptr < 200 && PL_oldoldbufptr != PL_oldbufptr && PL_oldbufptr != PL_bufptr)
        {
            /*
                    Only for NetWare:
        ...
        else if (PL_oldbufptr && PL_bufptr > PL_oldbufptr && PL_bufptr - PL_oldbufptr < 200 && PL_oldbufptr != PL_bufptr)
        {
            /*
                    Only for NetWare:
                    The code below is removed for NetWare because it
    ...
    case 0xFF:
        if (s[1] == 0xFE)
        {
            ...
            if (DEBUG_p_TEST || DEBUG_T_TEST)
                PerlIO_printf(Perl_debug_log, "UTF-16LE script encoding (BOM)\n");
#endif
            s += 2;
            if (PL_bufend > (char *)s)
            {
...
#ifdef DEBUGGING
            if (DEBUG_p_TEST || DEBUG_T_TEST)
                PerlIO_printf(Perl_debug_log, "UTF-16BE script encoding (BOM)\n");
#endif
            s += 2;
            if (PL_bufend > (char *)s)
            {
...
#ifdef DEBUGGING
            if (DEBUG_p_TEST || DEBUG_T_TEST)
                PerlIO_printf(Perl_debug_log, "UTF-16LE script encoding (no BOM)\n");
#endif
            s = add_utf16_textfilter(s, TRUE);
#else
            /* diag_listed_as: Unsupported script encoding %s */
	   ...
	   Read an extra octect if the buffer currently has an odd number. */
        while (1)
        {
		   ...
		   *coupled* with all the benefits of partial reads and
		   endianness.  */
                const U8 *const last_hi = (U8 *)SvPVX(utf16_buffer) + ((SvCUR(utf16_buffer) & ~1) - (reverse ? 1 : 2));
...
int Perl_keyword_plugin_standard(pTHX_ char *keyword_ptr, STRLEN keyword_len, OP **op_ptr)
{

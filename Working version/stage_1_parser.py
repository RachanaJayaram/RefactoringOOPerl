...
def p_var_dec( p ):
    '''var_dec :  NAME  ASSIGNOP term SEMI
                ...
                '''
    if p[1]=='my':
        if p[2][1:] not in  lookup_table[function_stack[-1]].keys():
            lookup_table[function_stack[-1]][p[2][1:]]=['my']
        else:
            lookup_table[function_stack[-1]][p[2][1:]].append('my')
    elif p[1]=='local':
        if p[2][1:] not in  lookup_table[function_stack[-1]].keys():
            lookup_table[function_stack[-1]][p[2][1:]]=['local']
        else:
            lookup_table[function_stack[-1]][p[2][1:]].append('local')
...
def p_name(p):
    '''name : NAME'''
    if p[1] not in  lookup_table[function_stack[-1]].keys():
        lookup_table[function_stack[-1]][p[1][1:]]=['global']

...
def p_package_dec(p):
    ...
    constants.package=1
    p[0]=("str_=lambda x: '' if x==None else str(x)","class "  + str(p[2]) + " :")
...
def p_not_package(p):
    ...
    global_variables=[variable + '=None' for variable in global_variables if variable not in ["end_","sep_"]]+[variable + "=''" for variable in global_variables if variable in ["end_","sep_"]]
...
def p_print_st(p):
    ...
    sep="''"
    if "end_" in lookup_table[function_stack[-1]]:
        if lookup_table[function_stack[-1]]["end_"]["last_used"]=="my":
            end="my_end_"
        if lookup_table[function_stack[-1]]["end_"]["last_used"]=="local":
            end="local_end_"
        if lookup_table[function_stack[-1]]["end_"]["last_used"]=="global":
            end="end_"
    if "sep_" in lookup_table[function_stack[-1]]:
        if lookup_table[function_stack[-1]]["sep_"]["last_used"]=="my":
            sep="my_sep_"
        if lookup_table[function_stack[-1]]["sep_"]["last_used"]=="local":
            sep="local_sep_"
        if lookup_table[function_stack[-1]]["sep_"]["last_used"]=="global":
            sep="sep_"

    p[0]="print( "+p[2]+",end="+end+",sep="+sep+" )"
